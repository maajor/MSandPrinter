...
volatile signed char Stepper::count_direction[NUM_AXIS] = { 1, 1, 1, 1, 1, 1};
...
#define S_APPLY_DIR(v,Q) S_DIR_WRITE(v)
#define S_APPLY_STEP(v,Q) S_STEP_WRITE(v)
...
void Stepper::isr() {
  ...
  if (!current_block) {
    ...
    if (current_block) {
	  ...
	  counter_X = counter_Y = counter_Z = counter_E = counter_W = counter_S = - (current_block->step_event_count >> 1);
...
#if HAS_S_ENABLE
	S_ENABLE_INIT;
	if (!S_ENABLE_ON) S_ENABLE_WRITE(HIGH);
#endif
 ...
 */
void Stepper::set_position(const long &a, const long &b, const long &c, const long &e, const long &w, const long &s) {
...
void Stepper::report_positions() {
  ...
  long xpos = count_position[X_AXIS],
	   ...
	   wpos = count_position[W_AXIS],
	   spos = count_position[S_AXIS];
  CRITICAL_SECTION_END;
  ...
  SERIAL_PROTOCOL(wpos);
  SERIAL_PROTOCOLPGM(" S:");
  SERIAL_PROTOCOL(spos);
