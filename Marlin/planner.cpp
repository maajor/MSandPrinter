...
long Planner::position[NUM_AXIS] = { 0,0,0,0,0,0 };
 ...
 */
void Planner::_buffer_line(const float &a, const float &b, const float &c, const float &e,const float &w,const float &s, float fr_mm_s, const uint8_t extruder) {
  ...
  long target[XYZE] = {
    ...
    lround(w * axis_steps_per_mm[W_AXIS]),
    lround(s * axis_steps_per_mm[S_AXIS])
  };
  ...
  long da = target[X_AXIS] - position[X_AXIS],
	   ...
	   ds = target[S_AXIS] - position[S_AXIS];
  /*
  ...
  uint8_t dm = 0;
    ...
    if (db < 0) SBI(dm, Y_AXIS);
    if (dc < 0) SBI(dm, Z_AXIS);
	if (dw < 0) SBI(dm, W_AXIS); //maajor
	if (ds < 0) SBI(dm, S_AXIS);
  #endif
  ...
  // See http://www.corexy.com/theory.html
    ...
    block->steps[Y_AXIS] = labs(db);
    block->steps[Z_AXIS] = labs(dc);
	block->steps[W_AXIS] = labs(dw); //maajor
	block->steps[S_AXIS] = labs(ds);
  #endif

  block->steps[E_AXIS] = esteps;
  //maajor
  block->step_event_count = max(MAX4(block->steps[X_AXIS], block->steps[Y_AXIS], block->steps[Z_AXIS], esteps), max(block->steps[W_AXIS], block->steps[S_AXIS]));
  //block->step_event_count = MAX4(block->steps[X_AXIS], block->steps[Y_AXIS], block->steps[Z_AXIS], esteps);
  //SERIAL_ECHOPAIR("max step: ", block->step_event_count);
  //SERIAL_EOL;
  ...
  //enable active axes
    ...
    if (block->steps[Y_AXIS]) enable_y();
      ...
      if (block->steps[Z_AXIS]) enable_z();
    #endif
	if (block->steps[W_AXIS]) enable_w();// maajor
	if (block->steps[S_AXIS]) enable_s();
  #endif
  ...
  delta_mm[W_AXIS] = dw * steps_to_mm[W_AXIS];
  delta_mm[S_AXIS] = ds * steps_to_mm[S_AXIS];
  ...
  if (!block->steps[X_AXIS] && !block->steps[Y_AXIS] && !block->steps[Z_AXIS]) {
    ...
    if (block->step_event_count <= cutoff_long) {
	 ...
	 // LIMIT_ACCEL_LONG(W_AXIS, 0);// maajor
	 // LIMIT_ACCEL_LONG(S_AXIS, 0);
...
void Planner::_set_position_mm(const float &a, const float &b, const float &c, const float &e, const float &w, const float &s){
  #if ENABLED(DISTINCT_E_FACTORS)
  ...
  long na = position[X_AXIS] = lround(a * axis_steps_per_mm[X_AXIS]),
       ...
       nw = position[W_AXIS] = lround(w * axis_steps_per_mm[W_AXIS]),
       ns = position[S_AXIS] = lround(s * axis_steps_per_mm[S_AXIS]);
  stepper.set_position(na, nb, nc, ne, nw, ns);
...
void Planner::set_position_mm_kinematic(const float position[NUM_AXIS]) {
  ...
  #else
    _set_position_mm(pos[X_AXIS], pos[Y_AXIS], pos[Z_AXIS], position[E_AXIS],position[W_AXIS],position[S_AXIS]);
  #endif
}
