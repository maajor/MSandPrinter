...
// W Stepper
#if ENABLED(HAVE_L6470DRIVER) && ENABLED(W_IS_L6470)
    extern L6470 stepperW;
#define W_ENABLE_INIT NOOP
#define W_ENABLE_WRITE(STATE) do{if(STATE) stepperW.Step_Clock(stepperW.getStatus() & STATUS_HIZ); else stepperW.softFree();}while(0)
#define W_ENABLE_READ (stepperW.getStatus() & STATUS_HIZ)
#define W_DIR_INIT NOOP
#define W_DIR_WRITE(STATE) stepperW.Step_Clock(STATE)
#define W_DIR_READ (stepperW.getStatus() & STATUS_DIR)
#else
#if ENABLED(HAVE_TMCDRIVER) && ENABLED(W_IS_TMC)
...
#else
#if ENABLED(HAVE_TMC2130DRIVER) && ENABLED(W_IS_TMC2130)
    extern Trinamic_TMC2130 stepperW;
#endif
#define W_ENABLE_INIT SET_OUTPUT(W_ENABLE_PIN)
#define W_ENABLE_WRITE(STATE) WRITE(W_ENABLE_PIN,STATE)
#define W_ENABLE_READ READ(W_ENABLE_PIN)
#endif
...
// S Stepper
#if ENABLED(HAVE_L6470DRIVER) && ENABLED(S_IS_L6470)
    extern L6470 stepperS;
#define S_ENABLE_INIT NOOP
#define S_ENABLE_WRITE(STATE) do{if(STATE) stepperS.Step_Clock(stepperS.getStatus() & STATUS_HIZ); else stepperS.softFree();}while(0)
#define S_ENABLE_READ (stepperS.getStatus() & STATUS_HIZ)
#define S_DIR_INIT NOOP
#define S_DIR_WRITE(STATE) stepperS.Step_Clock(STATE)
#define S_DIR_READ (stepperS.getStatus() & STATUS_DIR)
#else
#if ENABLED(HAVE_TMCDRIVER) && ENABLED(S_IS_TMC)
    extern TMC26XStepper stepperS;
#define S_ENABLE_INIT NOOP
#define S_ENABLE_WRITE(STATE) stepperS.setEnabled(STATE)
#define S_ENABLE_READ stepperS.isEnabled()
#else
#if ENABLED(HAVE_TMC2130DRIVER) && ENABLED(S_IS_TMC2130)
    extern Trinamic_TMC2130 stepperS;
#endif
#define S_ENABLE_INIT SET_OUTPUT(S_ENABLE_PIN)
#define S_ENABLE_WRITE(STATE) WRITE(S_ENABLE_PIN,STATE)
#define S_ENABLE_READ READ(S_ENABLE_PIN)
#endif
#define S_DIR_INIT SET_OUTPUT(S_DIR_PIN)
#define S_DIR_WRITE(STATE) WRITE(S_DIR_PIN,STATE)
#define S_DIR_READ READ(S_DIR_PIN)
#endif
#define S_STEP_INIT SET_OUTPUT(S_STEP_PIN)
#define S_STEP_WRITE(STATE) WRITE(S_STEP_PIN,STATE)
#define S_STEP_READ READ(S_STEP_PIN)
