...
class Planner {
  ...
  public:
     ...
     */
    static void _buffer_line(const float &a, const float &b, const float &c, const float &e, const float &lw,const float &ls,float fr_mm_s, const uint8_t extruder);

    static void _set_position_mm(const float &a, const float &b, const float &c, const float &e, const float &w, const float &s);
     ...
     *
     *  lx,ly,lz,e   - target position in mm or degrees// add lw, ls by maajor
     *  fr_mm_s      - (target) speed of the move (mm/s)
     *  extruder     - target extruder
     */
    static FORCE_INLINE void buffer_line(ARG_X, ARG_Y, ARG_Z, const float &e, const float &lw,const float &ls,const float &fr_mm_s, const uint8_t extruder) {
      #if PLANNER_LEVELING && IS_CARTESIAN
        apply_leveling(lx, ly, lz);
      #endif
      _buffer_line(lx, ly, lz, e, lw, ls, fr_mm_s, extruder);
    ...
    static FORCE_INLINE void buffer_line_kinematic(const float target[XYZE], const float &fr_mm_s, const uint8_t extruder) {
      ...
      #else
        _buffer_line(pos[X_AXIS], pos[Y_AXIS], pos[Z_AXIS], target[E_AXIS],target[W_AXIS],target[S_AXIS], fr_mm_s, extruder);
      #endif
    }
     ...
     */
    static FORCE_INLINE void set_position_mm(ARG_X, ARG_Y, ARG_Z, const float &e, const float &lw, const float &ls) {
      #if PLANNER_LEVELING && IS_CARTESIAN
        apply_leveling(lx, ly, lz);
      #endif
      _set_position_mm(lx, ly, lz, e, lw, ls);
    }
